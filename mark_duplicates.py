...
def cmd_mark_duplicates(config, fof):
    ...
    with open(config["paths"]["cmd_files"] + cmd_sh, "a") as cmd_file:
        ...
        for fi in fof:
            ...
            bam_name = fi.split("/")[-1]
            cmd_str = "java -jar /software/bin/picard.jar MarkDuplicates I={} O={}{}_sorted_mkdup.bam ASSUME_SORTED=TRUE REMOVE_DUPLICATES=FALSE CREATE_INDEX=TRUE M={}{}_duplicate_metrics.txt VALIDATION_STRINGENCY=LENIENT 2> {}{}_RemoveDuplicates.err".format(fi, config["paths"]["mapping_mkdup"], sample_name, config["paths"]["mapping_QC"], sample_name, config["paths"]["mapping_mkdup"], sample_name)
...
def run_parallel(config, cmd_sh):
    ...
    cmd = "parallel --joblog {}{} -j 4 :::: {}{}".format(config["paths"]["mapping_mkdup"], log_str, config["paths"]["cmd_files"], cmd_sh)
...
def write_output_fof(config):
    ...
    cmd_fof = 'find {} -name "*_sorted_mkdup.bam" > {}{}'.format(config["paths"]["mapping_mkdup"], config["paths"]["fof_files"], fof) 
